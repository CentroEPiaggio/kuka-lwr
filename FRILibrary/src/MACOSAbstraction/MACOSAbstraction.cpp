...
{
	timeval 				CurrentLocalMachineTime;

	gettimeofday(&CurrentLocalMachineTime, NULL);

	if ( (GetSystemTimeInSecondsCalledFirstTime) || (Reset) )
	{
		gettimeofday(&StoredSystemTimeInSeconds, NULL);
	...
	return((float)(((double)(CurrentLocalMachineTime.tv_sec
					...
					+	(double)(CurrentLocalMachineTime.tv_usec
					-	StoredSystemTimeInSeconds.tv_usec)
					*	(double)1e-6));
}

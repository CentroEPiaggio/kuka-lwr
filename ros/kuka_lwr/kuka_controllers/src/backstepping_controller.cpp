...
namespace kuka_controllers 
{
	...
	bool BacksteppingController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
	{
		...
		sub_gains_ = nh_.subscribe("set_gains", 1, &BacksteppingController::set_gains, this);
	...
	void BacksteppingController::command_configuration(const kuka_controllers::MultiPriorityTask::ConstPtr &msg)
	{
		step_ = 0;
		first_step_ = 1;
		cmd_flag_ = 1;	
	...
	void BacksteppingController::set_gains(const std_msgs::Float64MultiArray::ConstPtr &msg)
	{
		if(msg->data.size() == 3)
		{
			for(int i = 0; i < PIDs_.size(); i++)

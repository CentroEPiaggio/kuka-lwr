...
namespace lwr_controllers 
{
	...
	bool OneTaskInverseDynamicsJL::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
	{
		...
		{
		    ROS_ERROR_STREAM("OneTaskInverseDynamicsJL: No robot description (URDF) found on parameter server ("<<n.getNamespace()<<"/robot_description)");
		    return false;
		}

		if (!nh_.getParam("root_name", root_name))
		{
		    ROS_ERROR_STREAM("OneTaskInverseDynamicsJL: No root name found on parameter server ("<<n.getNamespace()<<"/root_name)");
		    return false;
		}

		if (!nh_.getParam("tip_name", tip_name))
		{
		    ROS_ERROR_STREAM("OneTaskInverseDynamicsJL: No tip name found on parameter server ("<<n.getNamespace()<<"/tip_name)");
		    return false;
		}
